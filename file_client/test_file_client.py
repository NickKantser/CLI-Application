...
class TestFileClient(unittest.TestCase):
    ...
    def test_default_of_options(self, mock_req):
        ...
        mock_req.get.assert_called_once_with('http://localhost:50051/file/2/stat')
        self.assertEqual(result.exit_code, 0)

    # Tests that http:// is in the address and if not it's added
    @patch('file_client.requests')
    def test_http_in_address(self, mock_req):
        result = self.runner.invoke(file_client.cli, ['--grpc-server=somelink', 'read', '1'])
    ...
    def test_valid_host_with_file_output(self, mock_req):
        ...
        mock_req.get.assert_called_once_with('http://localhost:50051/file/1/stat')
    ...
    # Test that if --backend=rest, then request address corresponds to the --base-url option
    @patch('file_client.requests')
    def test_backend_rest_and_base_url(self, mock_req):
        ...
        result = self.runner.invoke(file_client.cli, ['--backend=rest',
                                                      '--grpc-server=http://localhost:14000/',
    ...
    # Test that if --backend=grpc, then request address corresponds to the --grpc-server option
    @patch('file_client.requests')
    def test_backend_grpc_and_grpc_server(self, mock_req):
        mock_req.get.return_value.status_code = 200
        mock_req.get.return_value.content = b"{'filename': 'some.txt'}"

        result = self.runner.invoke(file_client.cli, ['--backend=grpc',
                                                      '--grpc-server=http://localhost:14000/',
                                                      '--base-url=http://localhost:15000/',
                                                      'read', '2'])
